...
/** @scope BodyBoard.CouchDataSource.prototype */ {
	...
	*/
	...
	getServerView : function(type,name){
		var path = '/' + type + '/_design/app/_view/' + name;
		return path;
	},
...
//====================================================================
  ...
  fetch: function(store, query) {
	...
	didFetchSystems : function(response, store, query){
		if(SC.ok(response)) {
			...
			console.log('Problem fetching systems');
	...
	didFetchLabels : function(response, store, query){
		if(SC.ok(response)) {
			...
			console.log('Problem fetching labels');
		    store.dataSourceDidErrorQuery(query, response);
		 }
	},
	didFetchAuthors : function(response, store, query){
		if(SC.ok(response)) {
			...
			console.log('Problem fetching authors');
		    store.dataSourceDidErrorQuery(query, response);
		 }
	},
	didFetchCaptions : function(response, store, query){
		if(SC.ok(response)) {
			...
			console.log('Problem fetching captions');
		    store.dataSourceDidErrorQuery(query, response);
		 }
	},
  ...
  retrieveRecord: function(store, storeKey) {
		...
		if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.System)) {
			...
			console.log('Retrieving system ',id);
			SC.Request.getUrl(this.getServerPath('systems',id))
			...
			if(SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Label)){
				var id = store.idFor(storeKey);
				console.log('Retrieving label ',id);
				SC.Request.getUrl(this.getServerPath('labels',id))
					.header('Accept', 'application/json').json()
				...
				if(SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Caption)) {
					var id = store.idFor(storeKey);
					console.log('Retrieving caption ',id);
					SC.Request.getUrl(this.getServerPath('captions',id))
						.header('Accept', 'application/json').json()
					...
					if(SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Author)){
						var id = store.idFor(storeKey);
						console.log('Retrieving author ',id);
						SC.Request.getUrl(this.getServerPath('authors',id))
							.header('Accept', 'application/json').json()
						...
						if(SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Account)){
							...
							SC.Request.getUrl(this.getServerPath('accounts',id))
								.header('Accept', 'application/json').json()
  	...
  	didRetrieveSystem: function(response, store, storeKey) {
		if (SC.ok(response)) {
			console.log('System retrieved');
			var dataHash = response.get('body').content;
			//store.dataSourceDidComplete(storeKey, dataHash);
		} else {
			console.log('Problem retrieving system');
	...
	didRetrieveLabel: function(response, store, storeKey) {
		if (SC.ok(response)) {
			...
			console.log('Problem retrieving label');
			store.dataSourceDidError(storeKey, response);
		}
	},
	didRetrieveCaption: function(response, store, storeKey) {
		if (SC.ok(response)) {
			...
			console.log('Problem retrieving caption');
			store.dataSourceDidError(storeKey, response);
		}
	},
	didRetrieveAuthor: function(response, store, storeKey) {
		if (SC.ok(response)) {
			console.log('Author retrieved');
			var dataHash = response.get('body').content;
			store.dataSourceDidComplete(storeKey, dataHash);
		} 	else {
			console.log('Problem retrieving author');
			store.dataSourceDidError(storeKey, response);
		}
	},
  ...
  updateRecord: function(store, storeKey) {
		...
		if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Author)) {
			...
			SC.Request.putUrl(this.getServerPath('authors',id)).json()
				.header('Accept', 'application/json')
			...
			if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Account)) {
				...
				var id = store.idFor(storeKey);
				var dataHash = store.readDataHash(storeKey);
				SC.Request.putUrl(this.getServerPath('accounts',id)).json()
					.header('Accept', 'application/json')
				...
				if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Label)) {
					...
					var id = store.idFor(storeKey);
					var dataHash = store.readDataHash(storeKey);
					SC.Request.putUrl(this.getServerPath('labels',id)).json()
						.header('Accept', 'application/json')
					...
					if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Caption)) {
						...
						var id = store.idFor(storeKey);
						var dataHash = store.readDataHash(storeKey);
						SC.Request.putUrl(this.getServerPath('captions',id)).json()
							.header('Accept', 'application/json')
  ...
  destroyRecord: function(store, storeKey) {
		...
		if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Caption)) {
			...
			SC.Request.deleteUrl(this.getServerPath('captions',id + "?rev=" + rev)).json()
				.header('Accept', 'application/json')
			...
			if (SC.kindOf(store.recordTypeFor(storeKey), BodyBoard.Label)) {
				console.log('destroying label record');
				var id = store.idFor(storeKey);
				//var rev = this._docsRev[id];
				var dataHash = store.readDataHash(storeKey);
				var rev = this.getDocRev(dataHash);
				SC.Request.deleteUrl(this.getServerPath('captions',id + "?rev=" + rev)).json()
					.header('Accept', 'application/json')
					.notify(this, this.didDeleteLabel, store, storeKey)
					.send();
				return YES;
			} else {
				return NO;
	...
	didDeletelabel: function(response, store, storeKey) {
		var couchRes = this.processResponse(response);
		if (couchRes.ok) {
